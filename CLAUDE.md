...
## 명령
- 설치(Python, 순서 중요): `pip install -e shared/python && pip install -e "ai[dev]"`
- 테스트:        `pytest`                          # 루트에서 실행, asyncio_mode=auto
- 린트·포맷 검사:  `ruff check . && black --check .`
- 포맷 적용:      `black . && ruff check --fix .`
- BE 실행:       `uvicorn app.main:app --reload`   # be 패키지 셋업 후
- FE 실행:       `flutter run`
- DB 마이그레이션:  `alembic upgrade head`           # be 셋업 후
