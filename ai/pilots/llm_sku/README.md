...
### 신뢰성 정책 (계측 — 2026-07-08 사전 선언)

- **출력 예산 `max_tokens=8192`** — 양 벤더 동일(공정성). 4096에서 상향: run2에서 JSON
  절단 3건(≥3 = 계측 결함 규칙 발동).
- **재시도 이원화** (계열별 카운터 분리 기록 — `common/retry.py`):
  - (a) 가용성(5xx·UNAVAILABLE·overloaded): **최대 3회 지수 백오프(2s→8s→30s)** — 인프라 노이즈
  - (b) 검증 실패(절단·스키마 불일치)·기타: **1회 유지** — 능력 신호, 관대화 금지
- **실패 분류**(run_report.md): 절단(검증)/5xx(가용성)/기타 컬럼 분리.
  절단 **≤2건 → 채점 인정 / ≥3건 → 계측 결함**으로 해당 run **전체** 재실행
  (실패 문서만 부분 재실행 금지 — 체리피킹). 5xx 잔존 실패도 재실행 사유.
...
| `run_report.md` | 실행 리포트(파싱 집계 + 실패 분류(절단/5xx/기타) + 계열별 재시도·토큰·지연 요약) |
