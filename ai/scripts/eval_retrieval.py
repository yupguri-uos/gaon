...
게이트: hit@5가 --min-hit5(기본 0.8) 미만이면 GATE FAIL + 종료코드 1. --min-hit5 0 = 게이트 해제.
"""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.8,
        help="hit@5 게이트 기준(기본 0.8 — 세션 백로그). 0이면 사실상 게이트 해제(관찰 모드)",
    )
...
async def main() -> int:
    ...
    # 게이트 판정(세션 백로그): 미달이면 종료코드 1 — CI·수동 재적재 검증에서 실패로 잡히게
    hit5_ratio = hits5 / total
    if hit5_ratio >= args.min_hit5:
        print(f"\nGATE PASS: hit@5 {hit5_ratio:.0%} ≥ {args.min_hit5:.0%}")
        return 0
    print(f"\nGATE FAIL: hit@5 {hit5_ratio:.0%} < {args.min_hit5:.0%}")
    return 1
