...
규칙:
- supplies: 준비물마다
  {name_ko(한국어 원문 표기 — 반드시 한국어로 적는다. 모국어·영어로 채우지 않는다),
   name_native(모국어 이름), explanation_native(무엇이고 왜 필요한지 모국어 설명),
   spec(치수·규격 수치만 — 예 '175mm'·'A4'·'2B'. '매일 지참' 같은 지참 빈도·조건·용도는
        spec이 아니라 explanation_native에 쓴다. 수치 규격이 없으면 null),
   ecommerce_keyword(쿠팡에서 검색할 한국어 키워드 —
   구매가 합리적인 실물에만. 학교가 배부하는 물품(교과서 등), 가정에 이미 있을 물건,
   원문이 불명확한 항목은 null)}.
  ecommerce_deeplink는 비워 둔다(시스템이 채운다).
- supplies에는 구매 가능한 실물만 넣는다(§17.11). 제출·작성 서류(신청서·동의서·조사서·확인서 등)는
  supplies가 아니다 — 그 할 일은 calendar_events(제출 마감)나 회신 초안으로 표현한다.
...
- 자녀 학년이 주어지면 그 학년 수준에 맞게 설명 난이도·권장 행동을 조정한다(§17.10).
- 출력 언어: name_native·explanation_native는 모국어(__LANG__).
  단 ecommerce_keyword는 반드시 한국어 — 쿠팡 검색용이므로 모국어 금지(팀 확정 2026-07-09).
...
def _render_lifestyle_user(data: LifestyleActionInput) -> str:
    ...
    lines = [
        ...
        f"[날짜] {dates}",
        f"[마감] {ext.deadline.isoformat() if ext.deadline else '없음'}",
        f"[자녀 학년] {data.child_info.grade if data.child_info else '미지정'}",
...
class LifestyleActionAgent(Agent[LifestyleActionInput, ActionCard]):
    ...
    async def _run(self, data: LifestyleActionInput) -> ActionCard:
        ...
        # 키워드가 null/빈 값(비구매 항목, §17.11 2단-a)이면 deeplink도 null로 확정.
        for supply in card.supplies:
            supply.ecommerce_deeplink = (
                coupang_search_url(supply.ecommerce_keyword) if supply.ecommerce_keyword else None
            )
        return card
...
- output_ko: 학부모가 복사해 바로 보낼 수 있는 완결된 경어체 한국어 메시지 한 편.
  ...
  · 교사가 어느 학생인지 알 수 있도록 학년·반을 중심으로 지칭한다(예: "3학년 2반 학부모입니다").
    이름은 학부모가 정한 별명일 수 있으므로 교사 식별 근거로 삼지 말고, 주어지면 보조적으로만 덧붙인다.
    주어지지 않은 정보는 지어내지 않는다.
